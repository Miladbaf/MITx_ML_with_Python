...
def compute_probabilities(X, theta, temp_parameter):
    ...
    """
    # Compute the raw scores (k, n)
    scores = np.dot(theta, X.T) / temp_parameter
    
    # Subtract the max score for numerical stability (broadcasting the maximum score across rows)
    ...
    # Compute the probabilities by normalizing with the sum of exponentials
...
def compute_cost_function(X, Y, theta, lambda_factor, temp_parameter):
    ...
    # Convert the labels Y to a one-hot encoded matrix
    k = theta.shape[0]
    Y_one_hot = np.zeros((k, n))
    Y_one_hot[Y, np.arange(n)] = 1
    
    # To avoid log(0) issues, use a small epsilon value
    epsilon = 1e-12
    log_probabilities = np.log(probabilities + epsilon)
    
    # Compute the cross-entropy loss
    cross_entropy_loss = -np.sum(Y_one_hot * log_probabilities) / n
...
def update_y(train_y, test_y):
    ...
    Returns:
        ...
        test_y_mod3 - (n, ) NumPy array containing the new labels (a number between 0-2)
                    for each datapoint in the test set
    """
    # Compute the new labels as mod 3 of the original labels
...
def compute_test_error_mod3(X, Y, theta, temp_parameter):
    ...
    # Compute the predicted labels modulo 3
